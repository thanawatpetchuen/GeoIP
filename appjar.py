...
#####################################
# Embedded Basemap to Tkinter
#####################################
    def addBasePlot(self, title, row=None, column=0, colspan=0, rowspan=0, width=None, height=None):
        self.widgetManager.verify(self.Widgets.Plot, title)
        self.__loadMatplotlib()
        if FigureCanvasTkAgg is False:
            raise Exception("Unable to load MatPlotLib - plots not available")
            ...
            if width is not None and height is not None:
                fig.set_size_inches(width,height,forward=True)
            axes = fig.add_subplot(111)  ## here

            # Basemap config
            self.m = Basemap(projection='merc',llcrnrlat=-80,urcrnrlat=80,\
                llcrnrlon=-180,urcrnrlon=180,lat_ts=20,resolution='c', ax=axes)
    ...
    #        canvas.get_tk_widget().pack(side=TOP, fill=BOTH, expand=1)
            ...
            # For select which one if error (grid or pack)
    #         canvas._tkcanvas.pack(side=TOP, fill=BOTH, expand=1)
    ...
    def setBasemap(self, title, file):
        # Your algorithm
        self.lonlat = 40.730610
        self.lonlon = -73.935242
        for line in file:
            xpt, ypt = self.m(float(line[0]), float(line[1]))
            lonx, lony = self.m(self.lonlon, self.lonlat)

            # print(xpt, ypt)
            # print("Yesy", lonx, lony)

            self.m.plot(xpt, ypt, 'bo')
            # self.m.drawgreatcircle(float(line[0]), float(line[1]), self.lonlon, self.lonlat, color='m')

        # Refresh for re-draw plot
        self.refreshPlot(title)
